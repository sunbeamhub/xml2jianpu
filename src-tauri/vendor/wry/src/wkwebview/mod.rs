...
pub fn platform_webview_version() -> Result<String> {
  // NSBundle.bundleWithIdentifier crashes on device when the app's
  // CFBundleDisplayName contains non-ASCII characters (Apple bug).
  // tauri-runtime-wry only checks `.is_ok()`, so a static version is enough.
  Ok("ios".into())
}
