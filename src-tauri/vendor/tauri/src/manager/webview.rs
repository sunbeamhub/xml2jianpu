...
// iOS 16's local-network TCC blocks Rust/reqwest (BSD sockets) to LAN IPs even
// after the Settings toggle is on. Safari/WKWebView can still fetch the same URL.
// Skip the custom-protocol proxy on iOS so the webview loads Vite directly.
pub(crate) const PROXY_DEV_SERVER: bool = cfg!(all(dev, mobile, not(target_os = "ios")));
