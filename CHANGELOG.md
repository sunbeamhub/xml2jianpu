# Changelog

本文件遵循 [Keep a Changelog](https://keepachangelog.com/zh-CN/1.1.0/)，版本号遵循 [Semantic Versioning](https://semver.org/lang/zh-CN/)。

## [Unreleased]

## [0.0.18] - 2026-09-26

### 功能

- 发版执行 `version:bump` 时检查本机 iOS 描述文件：已过期则停下并提醒用 Xcode 连真机重签；未过期则把 Base64 复制到剪贴板，供更新 `IOS_MOBILE_PROVISION`

### 修复

- 「关于」页气泡样式内，图标和文字的间距从gap改为margin，向下兼容 iOS 12

## [0.0.17] - 2026-09-25

### 功能

- 移动端的关于弹层按宽度切换气泡和底部面板形态

### 修复

- 关于弹层可以下拉关闭向下兼容 iOS 12

## [0.0.16] - 2026-09-24

### 修复

- 更新跳转后允许关闭「关于」

## [0.0.15] - 2026-09-24

### 功能

- 深色模式下，使用颜色反转突出显示当前选择的内置乐谱

## [0.0.14] - 2026-09-24

### 功能

- 关于页 UI 重绘

## [0.0.13] - 2026-09-23

### 修复

- 修复主题闪烁的缺陷

## [0.0.12] - 2026-09-23

### 功能

- 自定义渲染关于页面的markdown原文

## [0.0.11] - 2026-09-23

### 功能

- 移调弹窗 UI 重绘

### 修复

- 试听播放进度条拖动事件向下兼容 iOS 12
- 使用简谱时，试听只高亮音符本身
- 打开移调弹窗时，暂停自动滚动高亮音符到视口的逻辑

## [0.0.10] - 2026-09-22

### 功能

- 试听开始或进度离开开头后，「试听」换成暂停/继续和停止；停回开头后仍显示「试听」。按钮外框宽度不变
- 音高轮廓上，已播放的一段用强调色填实，并显示进度线
- 拖动进度时谱面不再跟着滚；松手后再跟随当前拍
- 网页和 App 底部居中增加「关于」。桌面跟顶部工具栏一起显隐；平板和手机跟现有菜单图标一起显隐。从关于页返回后，指针只是停在标题栏上时，底部「关于」不会一直留着
- 打开时对照 GitHub 上已发布的版本。有更新时「关于」右上角出现红点；当天点「稍后提醒」后红点消失，次日仍落后再出现
- 关于页显示当前版本、最新版本，以及两者之间的更新说明。网页点「立即更新」会刷新页面；桌面和 Android 打开对应安装包，需自行安装；iOS 提示改用网页

## [0.0.9] - 2026-09-22

### 功能

- 移调面板可试听当前谱：播放或暂停，在音高轮廓上拖动进度
- 音色为「电子」或「钢琴」；钢琴采样在第一次选用时加载
- 播放时简谱高亮当前拍（和弦与休止一起亮），五线谱显示光标。高亮离开视口中部时滚到中央；手动滚动后暂停跟随，过一会儿再继续
- 已进入固定调并改半音时，试听音高跟随当前移调；速度按谱面标记，含中途变速
- 多行谱只播放第一行谱，该行内的声部叠在一起；反复、跳房子和装饰音不播放
- 更新 README

## [0.0.8] - 2026-09-22

### 修复

- 自动主题改回纯 CSS `prefers-color-scheme`：不再用 lastScheme、取反或 settle 干预首屏。iOS 主屏幕 WebView 第一帧媒体查询不可信时，仍可能闪一下。

## [0.0.7] - 2026-09-21

### 修复

- iOS 主屏幕 PWA 系统浅色时，状态栏文字不再与浅色背景同色、难以辨认
- iOS 主屏幕 PWA 主题为「自动」时，第一次打开不再先闪成与系统相反的颜色

## [0.0.6] - 2026-09-21

### 修复

- iOS 主屏幕 PWA 主题为「自动」时，首次打开不再先闪成与系统相反的颜色

## [0.0.5] - 2026-09-21

### 修复

- iOS Safari 主题为「自动」时，首次打开不再先闪成与系统相反的颜色
- Win/Linux 桌面图标改为铺满圆角矩形（坞/任务栏体量恢复，标题栏不再发糊）

## [0.0.4] - 2026-09-21

### 修复

- 曲谱很短时，点击页面下方空白无法显隐顶部菜单；短页铺满视口，空白仍走点按逻辑
- 简谱进固定调但未改半音时，切到五线谱不再把移调图标高亮成已改谱
- 桌面图标改为预烘焙 squircle（透明边距），macOS Launchpad 不再铺满格子、直角方块

### 文档

- 说明从 GitHub 下载的 macOS 包能安装，但打开可能报「已损坏」：不是信任引导，需对「易谱」执行 `xattr -cr`

## [0.0.3] - 2026-09-21

### 修复

- 移动端点空白无法唤出或收起菜单图标
- 深色五线谱纯黑底、纯白字，改为与简谱同一套页面底色和文字色
- 共用字号档位下五线谱歌词偏大，按简谱 16px 对齐
- 短谱时五线谱靠左、简谱居中，改为五线谱同样整块居中
- iOS 12 / 旧 Android WebView 加载五线谱失败：OSMD 经 `?url` 原样注入含 `?.` / `??`，Safari 12 无法解析；构建时降级该脚本，并垫 `replaceChildren`
- 桌面悬停标题栏时切换简谱/五线谱，工具栏不再跟谱面左右缘跳动；收起后再出现仍按现规则对齐

## [0.0.2] - 2026-09-20

### 功能

- 新增内置专辑「电子琴启蒙1期」：《宝宝学走路》《小司机》
- 简谱 / 五线谱切换（OSMD），导出当前视图 PDF，记住记谱方式
- 五线谱 PDF 用 NotoSansSC（含斜体）；乐谱字形不改写

### 文档

- README 拆成「使用」与「开发」：用户侧写网页 / PWA / 安装包入口与功能用法，开发者侧写技术栈、分端调试与 fork 发布（含 iOS IPA 限制）
- 功能配图改为四组：菜单展开对比、浅色/深色主题、移调、桌面多列

## [0.0.1] - 2026-09-18

首个版本。将 MusicXML 转为简谱，可在网页、PWA 与原生客户端中预览并导出 PDF。

### 技术

- 前端：Vue 3、Vite 6；D3 绘谱，jsPDF、svg2pdf 导出矢量 PDF
- 客户端：Tauri 2 共用同一套前端，覆盖 Windows、macOS、Linux、Android、iOS
- 访问：GitHub Pages、腾讯云 EdgeOne、可安装 PWA，以及桌面、移动安装包
- 浏览器：不支持 IE；Safari、iOS 11+，iPadOS 13+；macOS Safari 11+（约 10.12+），亦可用 Chrome、Edge、Firefox；Android 建议 8+ 较新 Chrome、Edge、Firefox；Windows 10+；Linux 近两年常见发行版
- App：Android 7+（minSdk 24，release 面向系统 WebView ≈ Chrome 61）；iOS 13+
- 客户端上传曲谱、导出 PDF 走系统文件对话框

### 功能

- 上传本地 `.musicxml`、`.xml`，即时排成简谱
- 内置示例，按专辑分组（儿歌、三色绘恋等）
- 纸张：默认「设备」跟随屏宽，亦可预览、导出 A4、A3
- 换行：自动、原谱换行，或每行 2–6 小节
- 桌面响应式多列（最多 4 列）；平板、手机单列
- 字号；主题浅色、深色、跟随系统
- 缩放与平移（桌面 Ctrl/Cmd + 滚轮或捏合，移动端双指捏合）
- 导出矢量 PDF，按唱名 + 歌词整组分页，不拆行
- 固定调移调：按 `1=C` 重写唱名，半音升降，导出使用当前调
- 谱头：调号、拍号、速度、情绪、作词、译配、作曲
- 简谱记谱：唱名与歌词对齐，时值下划线按拍分组，八度点、升降与还原、附点、延音线、连线、休止符、小节线与终止线、连谱号
- 偏好记忆：曲目、纸张、换行、字号、主题、上次导出纸张
- PWA：可装到主屏幕；首次联网后可离线查看内置示例并导出（用户上传的谱不缓存）
